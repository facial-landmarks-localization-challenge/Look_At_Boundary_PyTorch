...
# TEST: base forward part
# hg = Hourglass()
# mp = MessagePassing()
# x_in = torch.rand(1, 256, 64, 64)
# y_out, msg = mp(x_in)
# print(y_out.size(), msg['A']['1'][0].size())

# TEST: estimator
# x_in = torch.rand(1, 1, 256, 256)
# es = Estimator(msg_pass=0)
# y_out = es(x_in)
# print(y_out[-1].size())

# TEST: regressor
# img_in = torch.rand(5, 1, 256, 256)
# x_in = torch.rand(5, 13, 64, 64)
# rg = Regressor(fuse_stages=4)
# y_out = rg(img_in, x_in)
# print(y_out.size())

# TEST: number of parameters
# es = Estimator()
# rg = Regressor()
# print('    Total params of es: %.2fM' % (sum(p.numel() for p in es.parameters()) / 1e6))
# print('    Total params of rg: %.2fM' % (sum(p.numel() for p in rg.parameters()) / 1e6))

# TEST: discriminator
# x_es = torch.rand(2, 13, 64, 64)
# d = Discrim()
# y_out = d(x_es)
# print(y_out.size())
