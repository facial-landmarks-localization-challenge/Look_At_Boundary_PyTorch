...
if __name__ == '__main__':
    ...
    allShapes = np.zeros((2*kp_num, length))
    for line_index, line in enumerate(annotations):
        ...
        for data_index in range(kp_num):
            ...
            allShapes[data_index+kp_num][line_index] = float(coord_y_after[data_index])
        
        # import cv2
        # for coord_index in range(kp_num):
        #     cv2.circle(
        #                pic_crop, 
        #                (int(allShapes[coord_index][line_index]), 
        #                 int(allShapes[coord_index+kp_num][line_index])),
        #                1,
        #                (0,0,255)
        #               )
    ...
    newIdx = pdb(use_dataset, allShapes, numBins)
    print(len(newIdx))
